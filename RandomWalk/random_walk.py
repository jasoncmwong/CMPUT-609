...
def rl_experiment(agent, n, alpha, gamma, sigma, sigma_factor, environment, num_episodes):
    """
    ...
    for i in range(num_episodes):
        ...
        ep_q = agent.q
        rmse[i] = calc_ep_rmse(ANALYTIC_SOLN, ep_q)  # Calculate RMSE for current episode number
...
def main():
    ...
    fixed_agent = EpisodicRandomAgent()
    ...
    for i in range(len(SIGMA)):
        ...
        for run in range(NUM_RUNS):
            rms_err[:, run, i] = rl_experiment(fixed_agent, N, ALPHA, GAMMA, sigma_val, 1, environment, NUM_EPISODES)
    ...
    # Perform experiment for NUM_RUNS times
    for run in range(NUM_RUNS):
        rms_err_ep[:, run] = rl_experiment(episodic_agent, N, ALPHA, GAMMA, 1, SIGMA_FACTOR, environment, NUM_EPISODES)
    ...
    # Perform experiment for NUM_RUNS times
    for run in range(NUM_RUNS):
        rms_err_freq_raw[:, run] = rl_experiment(freq_raw_agent, N, ALPHA, GAMMA, 1, SIGMA_FACTOR, environment, NUM_EPISODES)
